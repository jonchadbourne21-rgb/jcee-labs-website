...
const methodLayers = [
  {
    id: "01",
    name: "JCEE Assurance Method",
    role: "Method above mechanisms",
    copy: "The public-facing logic for how a claim earns trust and how authority remains bounded.",
  },
  {
    id: "02",
    name: "Assurance Playbook",
    role: "Teachable operating guide",
    copy: "Templates, review gates, and an adoption pattern for applying the method consistently.",
  },
  {
    id: "03",
    name: "Evidence & Judgment",
    role: "Observation and bounded conclusion",
    copy: "Captures observations, provenance, scoring, review, and conclusions without collapsing inference into fact.",
  },
  {
    id: "04",
    name: "Authority & Contract",
    role: "Permission stays separate",
    copy: "Expresses what an actor may do under explicit scope, state, freshness, delegation, and revocation constraints.",
  },
  {
    id: "05",
    name: "VOW Runtime",
    role: "Consequential execution and recovery",
    copy: "Preserves durable intent, ambiguity, recovery, effect identity, policy boundaries, and receipts.",
  },
  {
    id: "06",
    name: "Adapters & Integrations",
    role: "Real-system translation",
    copy: "Translate external systems into explicit observations and permitted actions without hiding uncertainty.",
  },
];
...
export default function AssurancePage() {
  ...
  return (
    <main className="program-page assurance-program-page" id="top">
      ...
      >
        <p className="eyebrow">
          <span /> METHOD · EVIDENCE · BOUNDED CONSEQUENCE
        </p>
        ...
        <p className="program-deck">
          A repeatable operating method for turning uncertain AI-era claims into
          bounded, reproducible, reviewable evidence without letting confidence
          outrun authority.
        </p>
        <div className="program-status-row">
          <span>METHOD · ABOVE THE MECHANISMS</span>
          <span>PUBLIC CLAIM · BOUNDED</span>
        </div>
        <div className="program-links">
          <a href="#assurance-loop">SEE THE 12-STEP LOOP <span>→</span></a>
      ...
      <section className="program-statement" aria-labelledby="assurance-method-title">
        ...
        <div className="program-statement-copy">
          <p>
            Claims, observations, decisions, and consequences are different
            things. The method preserves those differences instead of collapsing
            them into one confidence score or one unexplained green state.
          </p>
          <p>
            Hypotheses may be broad. Authority must remain bounded. Evidence
            decides what can be claimed; policy decides what can be done.
          </p>
        </div>
      </section>
      ...
      <section
        ...
        aria-labelledby="assurance-loop-title"
      >
        <div className="section-index light">
          <span>THE OPERATING METHOD</span>
          <span>QUESTION → CLAIM → EVIDENCE → CONSEQUENCE → RECEIPT</span>
        </div>
        <div className="partner-shared-layout">
          <div>
            <p className="partner-kicker">THE 12-STEP ASSURANCE LOOP</p>
            <h2 id="assurance-loop-title">A reusable path from question to bounded action.</h2>
            <p>
              Precommit what would count against the claim, preserve the object
              tested, attack uncertainty directly, and let the final boundary be
              no broader than the evidence.
            </p>
          </div>
          <ol className="partner-sequence">
            {assuranceLoop.map(([id, title, copy]) => (
              <li key={id}>
                <span>{id}</span>
                <strong>{title}</strong>
                <p>{copy}</p>
      ...
      >
        <div className="section-index">
          <span>ASSURANCE STACK</span>
          <span>METHOD ABOVE MECHANISMS</span>
        </div>
        <h2 id="assurance-architecture-title">
          One operating method. Distinct implementation responsibilities.
        </h2>
      ...
      <section className="program-boundaries" aria-labelledby="commercial-path-title">
        <div className="section-index">
          ...
          <span>OPEN LANGUAGE · PROTECTED IMPLEMENTATION</span>
        </div>
        <h2 id="commercial-path-title">The playbook can generate demand without being the whole product.</h2>
        <div className="partner-shared-layout">
          <div>
            <p>
              The governing direction is method first: teach a discipline people
              can understand and apply, then sell assessment, implementation,
              software, and ongoing assurance where repeatability and integration
              create real operating value.
            </p>
            <p>
              Public release decisions remain file-by-file and right-by-right.
              This page does not publish or release the internal working playbook,
              private code, sensitive verifier logic, or customer material.
            </p>
          </div>
          <ol className="partner-sequence">
            {commercialPath.map(([id, title, copy]) => (
              <li key={id}>
                <span>{id}</span>
                <strong>{title}</strong>
                <p>{copy}</p>
      ...
      <section className="program-statement" aria-labelledby="receipt-title">
        ...
        <div className="program-statement-copy">
          <p>
            A useful receipt identifies the intended claim or action, exact
            artifacts, authoritative observations, decision rule, effect identity,
            recovery state, limitations, and review pointer.
          </p>
          <p>
            It lets a skeptical reviewer reconstruct what the organization knew
            when it acted. That enterprise framing comes from the later edition;
            it sharpens the presentation without replacing the underlying method.
          </p>
        </div>
      </section>
      ...
      <section className="program-next">
        ...
        <p>
          JCEE VOW, QCS, JEC, JCEE Assurance, IEJ, and the Evidence Engine retain
          distinct responsibilities and evidence boundaries. Their composition
          must be tested; shared authorship does not make composition correct.
        </p>
        <div className="program-links">
          <a href="/registry">

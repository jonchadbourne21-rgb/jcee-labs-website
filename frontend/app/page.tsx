  ...
  return (
    <main className="min-h-screen bg-paper text-ink">
      ...
      <div className="mx-auto max-w-[1540px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        {stage === "approved" && <Approval form={form} claimId={claimId} totals={totals} analysis={analysis} onReset={reset} />}
      </div>
    </main>
  );
}
...
function Approval({ form, claimId, totals, analysis, onReset }: { form: ClaimForm; claimId: string; totals: { rcv: number; depreciation: number; acv: number; deductible: number; net: number }; analysis: unknown; onReset: () => void }) {
  const dossier = analysis as Record<string, unknown> | null;
  const assurance = dossier?.assurance as Record<string, unknown> | undefined;
  const journal = assurance?.journal as Record<string, unknown> | undefined;
  const evidence = assurance?.evidence as Record<string, unknown> | undefined;
  const runId = typeof assurance?.run_id === "string" ? assurance.run_id : "not recorded";
  const authorizationHash = typeof assurance?.authorization_sha256 === "string" ? assurance.authorization_sha256 : "";
  const downloadPath = typeof evidence?.download_url === "string" ? evidence.download_url : null;
  const verifyPath = typeof evidence?.verify_url === "string" ? evidence.verify_url : null;

  return <section className="mx-auto max-w-5xl pb-8"><div className="border-t-4 border-cyan bg-white p-6 shadow-sm sm:p-8"><div className="grid gap-8 lg:grid-cols-[1fr_310px]"><div><div className="mb-5 inline-flex h-12 w-12 items-center justify-center bg-teal-100 text-teal-900"><CheckCircle2 className="h-7 w-7" /></div><p className="text-xs font-bold uppercase tracking-[0.15em] text-cyan">Settlement authorized</p><h1 className="mt-2 text-3xl font-bold tracking-tight">Approval complete.</h1><p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">Claim <strong className="text-ink">{claimId}</strong> for {form.insuredName} cleared adjuster review and the VOW authorization boundary. Settlement instructions are ready for the payment workflow.</p>{assurance && <div className="mt-6 border border-teal-200 bg-teal-50 p-4"><div className="flex items-center gap-2 text-sm font-bold text-teal-950"><ShieldCheck className="h-5 w-5" />VOW 1.1 assurance verified</div><dl className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-2"><div><dt className="font-semibold text-slate-500">Run ID</dt><dd className="mt-0.5 break-all font-mono text-ink">{runId}</dd></div><div><dt className="font-semibold text-slate-500">Journal</dt><dd className="mt-0.5 text-ink">{String(journal?.event_count ?? "—")} events · chain intact</dd></div><div className="sm:col-span-2"><dt className="font-semibold text-slate-500">Authorization receipt</dt><dd className="mt-0.5 break-all font-mono text-ink">{authorizationHash || "not recorded"}</dd></div></dl>{downloadPath && verifyPath && <div className="mt-4 flex flex-wrap gap-2"><a href={`${API_URL}${downloadPath}`} className="inline-flex items-center gap-2 border border-teal-300 bg-white px-3 py-2 text-xs font-bold text-teal-950 hover:border-cyan"><FileCheck2 className="h-4 w-4" />Download signed evidence</a><a href={`${API_URL}${verifyPath}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border border-teal-300 bg-white px-3 py-2 text-xs font-bold text-teal-950 hover:border-cyan"><ShieldCheck className="h-4 w-4" />Verify receipt</a></div>}</div>}<div className="mt-6 border-l-4 border-safety bg-orange-50 px-4 py-3 text-sm"><span className="font-bold">Next action:</span> Confirm the insured&apos;s payment preference before disbursement.</div><button onClick={onReset} className="mt-6 inline-flex items-center gap-2 border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold hover:bg-slate-50"><RotateCcw className="h-4 w-4" />Start another claim</button></div><div className="bg-ink p-5 text-white"><div className="text-xs font-bold uppercase tracking-[0.14em] text-cyan">Net settlement</div><div className="mt-2 text-4xl font-bold">{USD.format(totals.net)}</div><div className="mt-5 space-y-2 border-t border-slate-700 pt-4 text-sm text-slate-300"><div className="flex justify-between"><span>ACV</span><span>{USD.format(totals.acv)}</span></div><div className="flex justify-between"><span>Deductible</span><span>−{USD.format(totals.deductible)}</span></div></div></div></div><Timeline /></div></section>;
}
